...
import { tap, switchMap, startWith, distinctUntilChanged, debounceTime } from 'rxjs';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule ,MatListModule, InputComponent, MatButtonModule, MatIconModule, MatProgressBarModule, MatDividerModule, MatMenuModule, SelectComponent],
...
export class KpiSideListComponent extends Unsubscriber {
  ...
  itemList: KpiListViewModel[] = [];
  listSize = 0;
  extraFields: ExtraField[] = [];
  constructor(){
    super();
    this._otherSubscription = this.kpiService.getExtraFields().pipe(
  ...
  loadMore() {
    this.frm.get('pageIndex')?.setValue(+this.frm.get('pageIndex')?.value + 1, { emitEvent: false })
    this._otherSubscription = this.kpiService.getByFilter(this.frm.value).subscribe(c => this.itemList.push(...c.data))
  }
  onScroll(event: any) {
