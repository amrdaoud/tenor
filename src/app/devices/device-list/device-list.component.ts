...
export class DeviceListComponent extends Unsubscriber {
  ...
  btnClicked(btnIndex: number) {
    if(btnIndex == 0) {
      this._otherSubscription = this.dialog.open(DeviceFormComponent, {panelClass: 'techteec-form-dialog'}).afterClosed().pipe(
        filter(bindingObject => bindingObject),
        switchMap(bindingObject => this.deviceService.addElement(bindingObject))
      ).subscribe(viewObject => this.changed(this.latestFilter))
    }
  }
  rowClicked(element: DeviceListViewModel) {
    this._otherSubscription = this.deviceService.getById(element.id).pipe(
      switchMap(viewObject => this.dialog.open(DeviceFormComponent, {panelClass: 'techteec-form-dialog', data: viewObject}).afterClosed()),
      filter(bindingObject => bindingObject),
      switchMap(bindingObject => this.deviceService.editElement(bindingObject))
    ).subscribe(viewObject => this.changed(this.latestFilter))
  }
}
